...
@section('content')
    <div x-data='enginesComponent()' class="flex flex-col items-center justify-center">
        <form class="mt-40 space-y-4 max-w-md sm:max-w-sm w-full">
            <h1 class="mb-2 text-2xl font-bold text-gray-800">Control Panel</h1>
            ...
            <div class="space-x-4">
                ...
                <button x-on:click.prevent="" class="w-24 py-3 text-white rounded-md"
                    :class="[isOn ? 'bg-green-600' : 'bg-red-600']" x-text="isOn ? 'On' : 'Off'"
                    x-on:click="toggleIsOn()"></button>
            </div>
        </form>
    </div>
